// --- CHANGED ---
// Import your two new models instead of the old one
import ExternalDelegate from '../../../../models/ext-del';
import InternalDelegate from '../../../../models/int-del';
// --- END CHANGE ---
...
    emailId: string;
    participantType: string; // This field is now used to route data
...
    // --- Task 1: Save to MongoDB (Refactored) ---
    // --- CHANGED ---
    // This logic now routes the data to the correct collection
    // based on the participant type, matching your admin panel.
    if (data.participantType === 'Internal') {
        // Ensure 'int-dels.ts' model schema includes all RegistrationData fields
        const newInternalDelegate = new InternalDelegate(data);
        mongoPromise = newInternalDelegate.save();
    } else {
        // Assume 'External', 'Individual', etc. all go to the external collection
        // Ensure 'ext-dels.ts' model schema includes all RegistrationData fields
        const newExternalDelegate = new ExternalDelegate(data);
        mongoPromise = newExternalDelegate.save();
    }
    // --- END CHANGE ---
